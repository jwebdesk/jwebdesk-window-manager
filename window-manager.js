...
// -----------------------------------------------------------------------------------------------------------------------    
    ...
    jwebdesk.WindowManager.Window = function (setup, restore_settings) {
        ...
        switch (layer) {
            case "window":
                ...
                this.component.on({
                    ...
                    "state":  function (n,e) { 
                        // console.log("win.state", arguments);
                        win.trigger(n,jwk.extend({}, e, { window: win, component: e.window}));
                    }
                }, this);
                break;
            case "west":
        ...
        this.set("icon", setup.getIcon(16)); 
    ...
    jwebdesk.WindowManager.Window.prototype.load = function (params) {
        ...
        if (setup.category == "app") {
            ...
            var app_url = setup.url_absolute();
...
// console.log("jwebdesk.WindowManager.Window.prototype.load()", "jwk.global._listeners.iframe:", [jwk.global._listeners.iframe]);
            ...
            (function (fake_proxy) {
                iframe.on("change:proxy", function(n,e) {                    
                    var proxy = fake_proxy;
                    console.log("change_id() ------------------> APP: ", setup.packid, "fake_proxy:", fake_proxy._proxy_id, "iframe_proxy", e.value._proxy_id);
                    fake_proxy.change_id(e.value._proxy_id);
                ...
                df.resolve(win, fake_proxy._proxy_id, setup.get("name"));
            })(jwk.global.proxy());
            
            
            /*
            iframe.one("change:proxy", function(n,e) {
                console.log("CHAMGE_ID ", fake_proxy._proxy_id , "->", e.value._proxy_id);
                fake_proxy.change_id(e.value._proxy_id);
            });
            */
            ...
            */
            ...
            if (app_url) {
                ...
                setTimeout(function (){                    
                    if (df.state() == "pending") {
                        var fake_proxy = jwk.global.proxy();
                        iframe.one("change:proxy", function(n,e) {                                            
                            // HACK:
                            // me pasó que al abrior el opfolders demora mucho el seteo del proxy y muere por timeout
                            fake_proxy.change_id(e.value._proxy_id);
                        });
